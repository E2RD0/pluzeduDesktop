...
namespace LoginForm.Database
{
    class funcionesCRUD
    {
        ...
        public static DataTable buscarContactosNivel(int tipo, string busqueda)
        {
            DataTable datos = new DataTable();
            MySqlCommand command = new MySqlCommand("SELECT u.id AS ID, u.nombres AS Nombres, u.apellidos AS Apellidos, u.username AS Usuario, u.email AS Email, u.clave AS Clave, u.codigo AS Codigo, t.nombre AS Tipo, e.nombre AS Estado FROM ((usuario u INNER JOIN usuarioestado e ON u.id_usuarioestado = e.id) INNER JOIN usuariotipo t ON u.id_usuariotipo = t.id) WHERE u.id_usuariotipo = '" + tipo + "' AND (u.nombres LIKE '%" + busqueda + "%' OR u.apellidos LIKE '%" + busqueda + "%') ORDER BY u.nombres ASC", conexion.obtenerconexion());
        ...
        public static int crearConversacionGrupal(int id1, List<int> array, string descripcion, string titulo)
        {
            //CREAR
            int idConv;
            int contador = 0;
            MySqlCommand CrearConversacion = new MySqlCommand("INSERT INTO conversacion(id_admin, titulo, descripcion, es_grupo) VALUES(@id1, @titulo, @descripcion, 1); SELECT LAST_INSERT_ID() FROM conversacion; ", conexion.obtenerconexion());
            CrearConversacion.Parameters.Add("@id1", MySqlDbType.Int32).Value = id1;
            CrearConversacion.Parameters.Add("@titulo", MySqlDbType.String).Value = titulo;
            CrearConversacion.Parameters.Add("@descripcion", MySqlDbType.String).Value = descripcion;
            idConv = Convert.ToInt32(CrearConversacion.ExecuteScalar());
            MySqlCommand InsertarAdmin = new MySqlCommand("INSERT INTO conversacionintegrantes(id_usuario, id_conversacion) VALUES('" + id1 + "','" + idConv + "')", conexion.obtenerconexion());
            InsertarAdmin.ExecuteNonQuery();
            for (int i = 0; i < array.Count; i++)
            {
                MySqlCommand InsertarUsuarios = new MySqlCommand("INSERT INTO conversacionintegrantes(id_usuario, id_conversacion) VALUES('" + array[i] + "','" + idConv + "')", conexion.obtenerconexion());
                InsertarUsuarios.ExecuteNonQuery();
                contador++;
            }
            //COMPROBAR Y RETORNAR
            if (contador == array.Count)
            {
                return 1;
            }
            else
        ...
        //REPORTE
        public static DataTable reporteUsuarios(int tipo)
        {
            DataTable datos = new DataTable();
            string instrucciones = "SELECT u.nombres AS Nombres, u.apellidos AS Apellidos, u.username AS Usuario, u.email AS 'Correo Electrónico', u.codigo AS 'Código', e.nombre AS Estado FROM ((usuario u INNER JOIN usuarioestado e ON u.id_usuarioestado = e.id) INNER JOIN usuariotipo t ON u.id_usuariotipo = t.id) WHERE u.id_usuariotipo = " + tipo;
            MySqlCommand comando = new MySqlCommand(instrucciones, conexion.obtenerconexion());
            MySqlDataAdapter adapter = new MySqlDataAdapter(comando);
            adapter.Fill(datos);
            return datos;
